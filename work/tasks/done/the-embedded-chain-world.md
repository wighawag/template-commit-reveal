...
## DONE FOR CLAUSE ONE, 2026-09-22; the chrome is what is left, and it is upstream work

The reference game plays a full round against a chain in the tab, on all four template nodes, gated by `web/e2e/tests/offline.e2e.ts`. `web/src/lib/game/core/advance.ts` is the `advanceCycle` caller (framework, inert under the timed policy), `web/src/lib/offline.ts` is this game's world, `/offline` is the route and the home page links to it. Measured in a browser against the production build: 363-394ms to a booted, deployed, connected and signed-in world, 137-166ms for a whole round.

**Clause two - the chrome names the world it is describing - is NOT met**, exactly as this file predicted it would be the harder half: the navbar, the account and the RPC banner live in `+layout.svelte`, outside every route subtree, and the five consumers that read the GLOBAL deployments store are mostly in `lib/core`, which every repo in this tree inherits. The offline route says so on screen rather than pretending otherwise. That is now the whole of what Phase 4 has left, and it is the item to reopen this task for.

Two things it found that were NOT predicted are written up as findings on this branch: a world's local signer broadcasting to the app's node (`a-worlds-signer-broadcast-to-the-apps-node.md`, a `lib/core` fix that belongs upstream) and a restored world being provisioned again, which made the stake refillable by pressing F5 (`a-restored-world-provisions-again.md`). The original brief follows.

## WHERE THIS STOOD, 2026-09-19: the mechanism is built, neither acceptance clause is met

Split on the line this file names at the bottom. `with/embedded-chain` exists in jolly-roger at `f9183ee`, UNPUSHED: `web/src/lib/embedded/` (chain id, node, deploy, records, world), `web/test/lib/embedded/` (18 tests, one of which boots a real chain, runs the real deploy and constructs a real context on it), and one shared-file edit (`core/connection/remote.ts`, an extraction). All six probes are answered by measurement and written up in the plan's Phase 4; the webevm-versus-hardhat gas question is answered there too (they agree). The ROUTE landed too (`/offline-demo`, measured: 264ms to a booted world, and three successive sends confirming in 31.4s / 0.6s / 1.1s in a real browser), so acceptance clause one holds for jolly-roger's own demo game. Playing it found a bug no gate could: the transaction observer is gated on a tab-leader election that was one per ORIGIN, so with two contexts in one tab the world's observer never ran and its transactions mined and stayed pending forever. Namespaced by chain now, with tests. Read that entry in the plan before adding anything else to a nested context: the general form is that `createContext` is per-world while some of what it builds is still per-origin. **MOSTLY SUPERSEDED, 2026-09-20: `integration`, the re-point, the cascade and the `variant/offline` deletion are all DONE and PUSHED.** What is NOT done is the chrome (clause two) and the reference game's own offline option, whose first prerequisite is item one below. The plan's "The split, and what it leaves" lists the five remaining pieces in order, each with what it now knows - start with number one, which is that **nothing in `web/src` calls `advanceCycle` and a manual round cannot complete without it.**
...
- **~~`variant/offline` is deleted LAST, after its one unique file moves.~~ BOTH DONE.** The move landed 2026-09-19 and the branch was deleted 2026-09-20, local and remote (recovery sha `6d4f458`). Verified before deleting: one commit ahead of `main`, one file in it, and all six of its section headings present in `README.embedded-chain.md`.
