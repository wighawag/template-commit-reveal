...
Step 1 also has to WRITE one file, because the framework advance client is useless without its game half: `web/src/lib/world/advance.ts`, reveal-or-die's own `getAttendance` read and `advanceCycle` send against its contract. The cascade offers the template's version as an add/add conflict there (git rename-detects `placement/advance.ts` onto `world/`), so it is a rewrite and not a resolution. **This is worth landing on its own merits regardless of the offline world**: `advanceCycle` has no caller in that repo either, so a manual deployment cannot complete a round there in ANY world.

Green after step 1: `pnpm install && pnpm --filter ./web check && pnpm --filter ./web run test:unit`, plus e2e.
