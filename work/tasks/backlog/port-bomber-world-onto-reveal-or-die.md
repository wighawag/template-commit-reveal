...
- **A BOMB'S DRAWN POSITION IS TAKEN ON TRUST, and bomber-world's own TODO says so; do not carry it silently.** A bomb may be placed anywhere along the path, and the contract places it at the avatar's position at that point in the action sequence (`resolution.currentX/Y`), ignoring the bomb action's `data`. The client draws other players' bombs at `bigIntIDToXY(action.data)` from `CommitmentRevealed`. For honest players the two always agree: the planner writes the path position into `data`, and a move blocked at reveal stops processing, and the event carries only `actions[0:numActionsResolved]`, so a bomb after a blocked step is never drawn either. What is open is a MODIFIED client: it can send a bomb whose `data` names another cell, the real bomb still lands on its path, and every other player sees it drawn somewhere else and can walk into the real one unwarned. **DECIDED 2026-09-26: the contract checks.** `_placeBomb` requires `action.data` to equal the current position (`PositionUtils.fromXY(resolution.currentX, resolution.currentY)`), so the event means what it says for ANY reader, an indexer included, and the client keeps drawing from `data` unchanged. The alternative (the client ignores `data` and derives the cell from the preceding move) was rejected because it fixes one reader and leaves the event lying to every other. What a mismatch DOES is part of the decision: treat it like a blocked move (`stopProcessing`, so the bomb and everything after it is not resolved and not in the event) rather than reverting, because a revert in the reveal phase costs the stake, and the rest of the reveal path already handles an unresolvable action by stopping. Pin it with a contract test: a reveal whose bomb names another cell resolves the actions before it, places no bomb anywhere, and emits only the resolved prefix.
