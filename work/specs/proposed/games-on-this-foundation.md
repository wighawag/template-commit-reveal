...
**BUILT 2026-09-09.** `with/pixi-js` exists, `main` carries no rendering library, and the three rows of the table below are now the tree's actual shape rather than a proposal. Four things the build settled or corrected:
...
- **The script compares `.ts` and nothing else**, by a hardcoded `grep '\.ts$'`, and says why: apps are expected to restyle their own wallet flows. That is right for a connection layer and does not transfer to a RENDER branch, where the shared files that matter include `routes/play/+page.svelte` and the canvas hosts, and where the branch's whole affordability rests on that page being byte-identical. Checked by hand instead this time - 158 shared `.svelte` files, none drifted - and covered going forward by `render-host-boundary.test.ts`, which is stronger than a diff for that one property and covers nothing else. Making the extension configurable upstream is the small fix; the trigger is a branch whose intended difference lives in a `.svelte` file, which `with/nft-identity` may well be since acquisition is a UI.
