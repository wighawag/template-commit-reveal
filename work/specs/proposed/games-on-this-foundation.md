...
### What the embedded world has to answer before it is built, and it is about the CLOCK

The chunked reveal makes this sharper than it was when the phase was written. `game/core/reveal-window.ts` sizes how much of a turn is openable from `averageBlockTime` read off the chain clock. An instant-mining world - which is what `miningConfig: {type: 'auto'}` gives, one block per transaction - has no meaningful block time, so the bound it computes is either wrong or vacuous, and a vacuous bound is the worse outcome: it says every turn fits, on the one world where nothing would have stopped it.

So an embedded world must declare what it reports as a block time rather than inherit whatever falls out of the miner, and the e2e that proves a full round in the tab has to be run against a world whose clock means something. This is the same shape as the three corrections recorded under "a turn may be bigger than a transaction": a property of one deployment read as a property of the mechanism.
