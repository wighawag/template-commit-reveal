...
**e2e WAS NOT RUN ON ANY NODE**, and that is the gap in this cascade. The change touches `context/core.ts` wiring, which is exactly what `verify` cannot see. Five nodes at about eight minutes each is the cost, and the nodes that most want it are `with/all` and reveal-or-die.
