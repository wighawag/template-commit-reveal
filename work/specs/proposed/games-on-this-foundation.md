...
**The order this forces is the point: put the genesis hash in the template's submission key BEFORE the embedded world exists.** Now it is a rename with nobody to break, and that is checked rather than assumed per AGENTS.md's own rule (`git ls-files contracts/deployments` is 0 files here). Afterwards it is a migration in a repo that by then has a second world able to invalidate the first world's records. It is NOT free in reveal-or-die, which has 13 committed records and whose own storage module already calls its prefix a wire. Full write-up in `work/notes/findings/an-embedded-world-has-two-persistences-and-only-one-is-keyed-to-the-chain.md`.
