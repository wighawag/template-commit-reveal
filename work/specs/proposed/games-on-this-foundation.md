...
1. **The client has to call `advanceCycle`** (probe 4). Nothing does. Without it a manual game cannot complete a round in any world, embedded or not, so this is framework work that the embedded world merely exposes first.
2. **The burner has to reach the in-tab node.** `initBurnerWallet({nodeURL})` types `nodeURL` as a `string`, and an embedded world has no URL. **Measured: the implementation already works with a provider object** - it builds its RPC with `createCurriedJSONRPC`, which takes either, and a burner constructed against a webevm node answers `eth_requestAccounts`, `eth_chainId` (the minted 9007199254740123, round-tripped correctly) and `eth_getBalance`. So this is a one-line type widening in `@etherkit/burner-wallet`, a package this project owns, and NOT a second wallet implementation.
3. **The route**, which is only the choosing: boot a world, provide it with a nested `<Context>`. `setAppContext` is svelte's `setContext` underneath, so a nested provider shadows its own subtree and nothing else - which is both the mechanism and acceptance clause two's bug.
4. **The chrome**, which is the hard half and is upstream work: the navbar and banners live in `+layout.svelte`, outside every route subtree, and the five consumers that read the GLOBAL deployments store rather than the context's are mostly in `lib/core`, which every repo here inherits.
5. **`integration`, the re-point, and the cascade**, which is a session of its own: two new verification nodes in jolly-roger, `stemBranch` moved in template-commit-reveal, five nodes to merge and e2e.
