...
**And the cascade found something nobody had checked:** `bleeps`, `mandalas` and `ronan-eth` have no `offshoot` config and therefore no `verify` command, so the fanout merged into all three completely ungated. That is Phase 0's own finding recurring in three repos. Verified by hand this time; they want configs.
