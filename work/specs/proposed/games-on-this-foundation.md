...
5. **The reference game's own offline option**, which is template-commit-reveal's and is a different thing from this demo: it needs the manual policy declared by an in-tab deploy (probe 5), the `advanceCycle` caller (1), and a provisioning hook that bonds an ERC20 stake.
6. **`integration`, the re-point, and the cascade**, which is a session of its own: two new verification nodes in jolly-roger, `stemBranch` moved in template-commit-reveal, five nodes to merge and e2e.
