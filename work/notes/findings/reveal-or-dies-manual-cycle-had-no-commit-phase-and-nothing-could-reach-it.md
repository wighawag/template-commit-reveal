...
3. **The contract change**: `CyclePolicy {Timed, Manual}` declared, `SKIP_COMMIT` DELETED, and the constructor refuses a configuration whose durations disagree with its policy. Backwards compatible with what is deployed, which is a claim that was CHECKED (`git ls-files contracts/deployments` says rise-testnet is live, its linked data predates the parameter, and the client reads such a deployment the old way - which is what it actually ran).

   Four contract tests, the manual branch's first exercise ever, checked for teeth by putting the old `commiting: false` back: two of the four fail and the other fifteen pass.

   **The first draft of this split `skipCommit` into a flag of its own, and that was wrong.** The TODO invites it, and it is the obvious reading of "allow to specify it separately". But `8548f63d` in this repo had already faced the identical decision and gone the other way, in as many words: *"`SKIP_COMMIT` is gone rather than formalised. It lost its only consumer, and it was derived from the same zero durations that meant 'manual', so one derivation stood for two unrelated things."* The same was true in reveal-or-die and it was checkable in one grep - every environment declared it false, and its only reads were the manual-cycle machinery the derivation had entangled it with. Formalising it would have kept a knob nobody turns AND given a descendant a concept its parent had deliberately removed, which is divergence-by-invention: the worst of the two, because a future cascade has nothing to conflict against.

   **The general form, and it is the sharper half of this whole note: when a descendant has the ancestor's old bug, look up what the ancestor DID about it, not just that it fixed it.** A parent's fix carries a decision about what to keep as well as what to change, and the second half is the one a reimplementation silently drops. Here the split was found by the maintainer asking "I thought we removed SKIP_COMMIT in TCR?" - which `git log -S 'SKIP_COMMIT'` answers in one command and which nothing in the descendant could have prompted.
