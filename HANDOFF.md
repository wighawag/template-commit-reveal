...
| `template-commit-reveal` | **29 commits ahead of its stem, fully merged with it, and PUSHED. Working tree clean.** All four suites green. **As of 2026-09-22 it also PLAYS OFFLINE**: `lib/offline.ts` is the reference game's own world on a chain in the tab, `game/core/advance.ts` is the `advanceCycle` caller a manual cycle needs, and `/offline` is the route. Four nodes, 1615-1632 server units, 53 e2e each |
