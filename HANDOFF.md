...
reveal-or-die, measured the same day for the same reason, since Phase 0 of `work/specs/proposed/games-on-this-foundation.md` cascades into it: `contracts:test` 9 passing (9 nodejs), `web:check` clean, `test:unit` 1618 in 128 files plus 72 in 11, `test:e2e` 49 passing, `contracts lint` 67 errors (pre-existing, same shape as the 29 here), `format:check` green in `web` and 7 files short in `contracts` (pre-existing).
