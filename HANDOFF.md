...
| `template-commit-reveal` | **29 commits ahead of its stem, fully merged with it, and PUSHED. Working tree clean.** All four suites green. **As of 2026-09-22 it also PLAYS OFFLINE, WITH OTHER PLAYERS IN IT, AND HAS A LOBBY**: `lib/offline.ts` is the reference game's own world on a chain in the tab, `game/core/advance.ts` is the `advanceCycle` caller a manual cycle needs, `/offline` is the route, and `lib/offline-players.ts` is the members the world plays so that the cycle has somebody to wait for. **HALF OF THAT IS FRAMEWORK AS OF 2026-09-24, once a second game had one too**: the lobby and the seat model are `game/lobby/{lobby,seats}.ts` (the lobby takes "is a game already here", "set one up around this table" and "forget it" as parameters, because it used to import the reference game's world builder by a name two games only coincidentally shared), the keyring and the pass loop are `game/core/played.ts`, the played seat's secret is `game/core/secret.ts`, and registering this browser's key without a dialog is `game/acquire/authorise.ts` beside the version that asks. What is left in the app is the wiring, the words, and the three things the two games disagreed about - turn derivation, chunked reveal, and what settling costs. Four nodes, 1654-1671 server units, 53 e2e each |
...
- `pnpm --filter ./web test:unit` -> **1363 passing in 111 files (server) plus 65 in 10 files (client)**, as of the liveness backport. Was 1348 in 110 before Phase 1's items, and 744 in 66 before that. Note the script is two vitest projects (`--project server` then `--project client`) and prints two summaries, so a reader who takes the last one and stops has just recorded 65 as the suite.
...
reveal-or-die, measured the same day for the same reason, since Phase 0 of `work/specs/proposed/games-on-this-foundation.md` cascades into it: `contracts:test` 9 passing (9 nodejs), `web:check` clean, `test:unit` **1600 in 127 files plus 65 in 10** after Phase 1 (it was 1618 in 128 plus 72 in 11 before Phase 0 removed the duplicated input suite), `test:e2e` 49 passing, `contracts lint` 67 errors (pre-existing, same shape as the 29 here), `format:check` green in `web` and 7 files short in `contracts` (pre-existing).
...
- **`web/src/lib/deployments.ts` is GENERATED and gitignored, and this repo commits no deployment records, so the install-time fallback cannot help you.** After ANY contract change and after EVERY branch switch it must be rebuilt, or `check` fails naming contracts that do not exist on that branch. The recipe, which is worth keeping as a script because you will run it a dozen times in a cascade: export `MNEMONIC` and `MNEMONIC_localhost` as the junk mnemonic and `ETH_NODE_URI_localhost` at your port, `rm -rf contracts/deployments/localhost`, start `pnpm run node:local --port <port>` from `contracts/` under `setsid`, wait for `eth_chainId` to answer, then `pnpm compile && pnpm run deploy localhost --skip-prompts --no-compile && pnpm export localhost --ts ../web/src/lib/deployments.ts`, then kill the node's process GROUP. Note the scripts are the CONTRACTS package's, not the root's: at the root they are `contracts:compile`, `contracts:deploy` and `contracts:export`, and `pnpm compile` there fails with "Command \"compile\" not found". **Also `pnpm i` after a branch switch**: `with/pixi-js` and `with/all` carry dependencies the others do not, and without it `check` dies inside `vite.config.ts` on a missing `@assetpack/core` rather than on anything to do with your change.
- **NEVER `git stash` inside a conflicted merge.** It clears `MERGE_HEAD` and `git stash pop` does not restore it, so the commit you make afterwards has ONE parent and git no longer knows the cascade landed. Every gate passes, because the tree is right; the next merge into that repo replays the entire upstream history as conflicts. `git log -1 --format='%h parents: %p'` is the one-command check and belongs in the ritual beside the suites. See `work/notes/findings/git-stash-during-a-merge-silently-loses-the-second-parent.md`, including how to repair it without redoing the resolution.
...
- **A DEV RUN AND A BUILD ARE DIFFERENT WORLDS FOR THE SIGNER, and the dev one is the forgiving one.** `pnpm --filter ./web dev` started by hand carries no `PUBLIC_NODE_URL`, so the app has no RPC of its own and the local signer falls back to the connection's provider. A production build made with `pnpm web:build localhost` has one, and everything that broadcasts uses it. That is what hid a world's signer posting to the app's node for a whole session of green dev runs (see the finding of that name): **if what you are testing is where a transaction GOES, test the build.** `pnpm web:build localhost` then `npx vite preview --port <port> --strictPort`, started under `setsid` in its own command, and drive it with the `playwright` that `web/node_modules` already has - a script at `/tmp` cannot resolve it, so put the driver in `web/` and delete it afterwards.
- **`vite preview` CACHES ITS FILE LISTING, so a rebuild under a running preview serves 404s for every hashed asset.** It is `sirv` with `dev: false` underneath, which resolves the directory once at startup. The symptom is a page that loads, logs two 404s and then never renders, which reads exactly like an app bug and is not one: RESTART the preview after every `pnpm web:build`. This is the specific version of the stale-output-dir entry above, met while measuring three builds of the offline world in one session.
- **A DRIVER FOR THE BROWSER MEASUREMENTS IS DESCRIBED HERE BECAUSE THE SCRIPTS ARE THROWN AWAY.** Build with `pnpm web:build localhost`, start `npx vite preview --port <port> --strictPort` from `web/` under `setsid` IN ITS OWN COMMAND, and drive it with a `.cjs` file inside `web/` (one in `/tmp` cannot resolve `playwright`). What a lobby-era run measures: a fresh `browser.newContext()` per run so the visit is a FIRST visit; page load until `How many at the table?`; `[data-testid="seats-N"]` then `[data-testid="sit-down"]` until the strip AND `offlineWorld.context.delegation` reports `allowed` (a world is not playable before that, and waiting only for the strip measures a world still setting itself up); `page.reload()` until the strip, for the restore; then, per round, a canvas click, `Commit now`, and `cycle N+1`. Take at least four rounds: the FIRST round after a restore is not representative from four seats up. Delete the script afterwards.
- **`pkill -f "vite preview --port NNNN"` KILLS THE SHELL THAT RAN IT**, because that shell's own command line contains the pattern. The command produces no output and everything after the `pkill` in the same line silently does not happen - which in this session meant a file believed deleted was still there and got staged. Match on something the invoking command line does not contain, or take the pid from `lsof -ti :NNNN` first.

- **`contracts/dist` is a build input now, not just a publish artifact.** `web/src/lib/offline.ts` imports the deploy scripts and the rocketh config out of the contracts package, and every one of those export paths resolves into `dist`. `prepare` builds it (`pnpm compile && tsc`), so an ordinary `pnpm i` is enough; a worktree where `check` fails with module-resolution errors naming `template-commit-reveal-contracts` wants an install, not a deploy. Note `prepare` runs `tsc` and NOT `pnpm typescript`, which also type-checks `test/js`: that half fails on `with/nft-identity` today, and gating an install on a suite's types would make `pnpm i` impossible there.
...
- `tokens/StakeSale.sol` - one payable call that mints, stakes for `player` and
  forwards a gas stipend to the key that will play. It is what makes getting
  set up ONE transaction, and what gives the acquisition rail a consumer here.
  A real game replaces it with whatever actually gates entry; the client rail
  above it only needs one call that carries a value and forwards a stipend
...
- `web/src/lib/placement/` is the GAME. A descendant deletes this whole directory and writes its own; the split is by directory precisely so "what do I replace" has an obvious answer. `cells.ts` (zone maths mirroring `PositionUtils`), `state.ts` (camera-scoped reader), `view.ts` (the merge), `planning.ts` (clicks to a plan), `commit-reveal.ts` (the adapter), `reserve.ts` (what is at stake, read and withdrawn), `acquisition.ts` (how one is acquired: the arguments to `StakeSale.purchase`, and nothing else), `storage.ts` (the secret across reloads), `render/`, `ui/`.
- `web/src/lib/game/acquire/` is framework too: acquiring what lets you play, in one transaction, with a gas stipend to the signer, recoverable after a reload. It is the shape every game shares; WHAT is acquired is one value the game hands it. See its README.
...
- The setup gate asks for the STAKE first, and it used to be the other way round. The reason for the old order was that both were wallet transactions and a player who abandons setup half way should have spent as little as possible. That reason survives and now points the other way: staking goes through the acquisition rail (`$lib/game/acquire`), which is ONE transaction that stakes AND funds the signer's gas, and the signer then registers itself out of that stipend. So staking IS authorising, and asking to authorise first would demand a transaction the next step includes. The `authorise` step is still reachable, for a player who has a stake already and is opening a second browser: they have nothing left to buy, so the top-up flow (register and fund in one) is the remedy, which is the same flow the out-of-gas remedy uses.
...
- **A WORLD'S CONNECTION HAS TO CARRY ITS OWN NODE URL, and `lib/core` does not know that yet.** `createContext({establishConnection})` makes the connection a parameter, but `createCoreContext` still builds the LOCAL SIGNER's transport from the app's `PUBLIC_NODE_URL`, so a world's moves are broadcast to the app's chain. Fixed here by adding `nodeURL` to `EstablishedConnection` (the factory reports what it used; an embedded world reports nothing and the signer falls back to the connection's provider), and it is jolly-roger's bug rather than this repo's: the parameter is theirs and so is every consumer of the url. It cannot fire on `with/embedded-chain` alone, which targets `WalletConnected` and has no signer, which is exactly why it was still there. See the finding of that name.
...
2. **Port bomber-world** onto reveal-or-die. ~~Small: ~18 files differ, and 64 of 81 shared web files are byte-identical.~~ **That price is stale by a whole rebuild**: it was taken before reveal-or-die was ported, and bomber-world never was. Measured 2026-09-25: 91 files under `web/src` against reveal-or-die's 450, 143 paths in only one of the two, 28 common paths differing and NONE identical, with 29 commits of shared ancestry, so it is a merge rather than a graft. bomber-world also has no `stem` remote and no offshoot config, so it has no verify command yet; giving it one is the first step.
...
7. ~~**The credits config in `contracts/rocketh/config.ts` is still not set.**~~ **SET, and re-measured since, 2026-09-17.** `creditsGasMultiplier` went on the 31337 entry with the credits work and was then re-measured when the reveal became CHUNKED, because a credit stopped being one turn: a turn is now opened in `ceil(actions / actionsPerReveal)` transactions, so a credit is one commit plus one reveal STEP and a long turn honestly costs more than a short one. 750,000 on `main` (116,898 + 535,561 worst case, plus a margin) and 550,000 on the identity branches, which measure lower because a placement costs nothing there and a reveal writes no stake. The original note follows, because the trap it records is still worth knowing about.
   **The credits config in `contracts/rocketh/config.ts` is still not set.** It used to be worse than unset: upstream added a real, live `chains` block to that file, while the credits template sat further down as a SECOND commented-out `chains:` key in the same object literal, so uncommenting it as it stood was a duplicate key that would silently have replaced the live one. That trap is gone; the guidance now lives as a comment on the live block. What remains is a measurement: put `creditsGasMultiplier` (the worst-case gas of one move, which here is a commit plus a reveal) and optionally `creditsPerTopUp` on the 31337 entry. `web/src/lib/core/connection/credits.ts` reads exactly those and falls back to native currency without them, which is why half a configuration is worse than none.
8. ~~**The mint/approve/stake sequence is still three silent transactions.**~~ **GONE, and not by building the stepper.** A stepper narrates three transactions; the acquisition rail removes two of them. `StakeSale.purchase` is one payable call that mints, stakes for `player` and forwards a gas stipend to the local signer, `$lib/game/acquire` is the client rail over it (payer choice, consent, one send, the signer registering itself, and recovery from the operations ledger after a reload), and `reserve.fund()` is deleted along with the only code in the app that sent three transactions for one intention. What the player signs is one thing, and the dialog in front of it says who is paying and how much. The rail is framework and lives beside the round; what this game supplies is `placement/acquisition.ts`, which is the arguments and nothing else.
...
**A TURN CAN NOW BE HALF REVEALED, and this section still holds, on purpose.** A turn longer than `actionsPerReveal` is opened in several transactions, so a commitment can sit with some of its chunks applied and the rest never arriving. `acknowledgeMissedReveal(player)` closes that in ONE call and takes no chunk and no secret: the bond falls by each chunk's cost as it lands, so what is left earmarked is exactly what was never opened, and the contract needs nothing from the caller to settle it. Requiring the secret would have made settling depend on the very thing that failed - and since anyone may settle anyone's missed reveal here, a settlement only the absent player could perform is not one. The reasoning and the rejected alternatives are ADR-0002 (this branch); the two tests are "settles a half-revealed turn in ONE call, and frees the next cycle" and "will not let the cycle close on a half-revealed turn", the second being where the stranding hazard actually lives in this framework.
...
1. ~~**The round never reconciles with the chain.**~~ **CLOSED for the framework and for the reference game, 2026-09-08** (D10 in the plan on the `work` branch). `round.adopt(persisted)` is the whole of the framework's part, and it adds no `RoundState` member on purpose: a reconstructed round IS a restored round and nothing downstream may be able to tell. The rest is the game's, in the game's own directory - the chain read that `missed-reveal.ts` was already making and throwing away for the LIVE case, and whichever of three routes recovers the actions. The reference game asks the PLAYER and lets the hash judge, because a turn there is any subset of an open board and there is nothing to enumerate; that is the general route and it is what conquest would need too. **CLOSED for reveal-or-die too**, by SEARCH rather than by asking: it enumerates the walks the maze allows and hashes them until one matches, so the player is asked nothing. Its space is 3,000 to 16,000 candidates for a ten-move turn and about 85 microseconds a hash - not the "about 125" this document and D9 both used to say, which came from a three-move limit the game has never shipped with. An ENTRY is still not searchable (any cell on an unbounded map), so both routes exist there.
