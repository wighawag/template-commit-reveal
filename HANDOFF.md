...
| `template-commit-reveal` | **29 commits ahead of its stem, fully merged with it, and PUSHED. Working tree clean.** All four suites green. **As of 2026-09-22 it also PLAYS OFFLINE, WITH OTHER PLAYERS IN IT**: `lib/offline.ts` is the reference game's own world on a chain in the tab, `game/core/advance.ts` is the `advanceCycle` caller a manual cycle needs, `/offline` is the route, and `lib/offline-players.ts` is the two members the world plays so that the cycle has somebody to wait for (one waited-for member satisfies unanimity by existing). Four nodes, 1623-1640 server units, 53 e2e each |
