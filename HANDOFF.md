...
| `template-commit-reveal` | **29 commits ahead of its stem, fully merged with it, and PUSHED. Working tree clean.** All four suites green. **As of 2026-09-22 it also PLAYS OFFLINE, WITH OTHER PLAYERS IN IT, AND HAS A LOBBY**: `lib/offline.ts` is the reference game's own world on a chain in the tab, `game/core/advance.ts` is the `advanceCycle` caller a manual cycle needs, `/offline` is the route, `lib/offline-players.ts` is the members the world plays so that the cycle has somebody to wait for, and `lib/offline-seats.ts` plus `lib/offline-lobby.ts` are where a player chooses how many are at the table before it boots. There is no gate between choosing and playing: `lib/offline-authorise.ts` registers this browser's key at sign-in, through the same two functions the button used to. Four nodes, 1639-1656 server units, 53 e2e each |
